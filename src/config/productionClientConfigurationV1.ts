...
export class ProductionClientConfigurationErrorV1 extends Error {
  ...
  constructor(code: ProductionClientConfigurationErrorCodeV1) {
    super(code);
    this.name = "ProductionClientConfigurationErrorV1";
    this.code = code;
  }
}
...
export interface ProductionClientConfigurationV1 {
  ...
  readonly appCheckEnabled: false;
}
...
export function resolveProductionClientConfigurationV1(
  ...
  const values = Object.fromEntries(
    PRODUCTION_CLIENT_REQUIRED_VARIABLES_V1.map((name) => [
      name,
      requiredValue(source, name),
    ]),
  ) as Record<ProductionClientRequiredVariableV1, string>;

  if (values.VITE_AURA_RUNTIME_ENVIRONMENT !== "PRODUCTION") {
    throw new ProductionClientConfigurationErrorV1(
      "PRODUCTION_CLIENT_ENVIRONMENT_MISMATCH",
    );
  }
  if (values.VITE_FIREBASE_PROJECT_ID !== PRODUCTION_CLIENT_PROJECT_ID_V1) {
    throw new ProductionClientConfigurationErrorV1(
      "PRODUCTION_CLIENT_PROJECT_MISMATCH",
    );
  }
  if (values.VITE_FIREBASE_AUTH_DOMAIN !== PRODUCTION_CLIENT_AUTH_DOMAIN_V1) {
    throw new ProductionClientConfigurationErrorV1(
      "PRODUCTION_CLIENT_AUTH_DOMAIN_MISMATCH",
    );
  }
  ...
  return Object.freeze({
    ...
    authDomain: PRODUCTION_CLIENT_AUTH_DOMAIN_V1,
    projectId: PRODUCTION_CLIENT_PROJECT_ID_V1,
...
export function assertProductionClientDomainV1(hostname: string): void {
  if (hostname.trim().toLowerCase() !== PRODUCTION_CLIENT_DOMAIN_V1) {
    throw new ProductionClientConfigurationErrorV1(
      "PRODUCTION_CLIENT_DOMAIN_MISMATCH",
    );
  }

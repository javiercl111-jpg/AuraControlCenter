...
function safeEnvironment(environment = process.env) {
  return Object.fromEntries(
    Object.entries(environment).filter(
      ([key]) =>
        !/(?:CONTROL_PROOF|CAPABILITY_BEARER|SESSION_BEARER)/iu.test(
          key,
        ) &&
        key.toUpperCase() !==
...
const PREVIEW_CERTIFICATION_ENV_KEYS_V1 =
  Object.freeze([
    "VITE_AI_UX_02D2E4_CONTROL_PROOF_DIGEST_V1",
    "VITE_AURA_RUNTIME_ENVIRONMENT",
  ]);

function previewCertificationEnvironmentV1(
  baseEnvironment,
  transientEnvironment,
) {
  if (
    !transientEnvironment ||
    typeof transientEnvironment !== "object" ||
    Array.isArray(transientEnvironment)
  ) {
    fail("D2E4D_PREVIEW_CERTIFICATION_ENV_REJECTED");
  }

  const keys =
    Object.keys(transientEnvironment).sort();

  if (
    keys.length !== PREVIEW_CERTIFICATION_ENV_KEYS_V1.length ||
    keys.some(
      (key, index) =>
        key !== PREVIEW_CERTIFICATION_ENV_KEYS_V1[index],
    ) ||
    transientEnvironment.VITE_AURA_RUNTIME_ENVIRONMENT !== "PREVIEW" ||
    !/^[a-f0-9]{64}$/u.test(
      transientEnvironment.VITE_AI_UX_02D2E4_CONTROL_PROOF_DIGEST_V1 ?? "",
    )
  ) {
    fail("D2E4D_PREVIEW_CERTIFICATION_ENV_REJECTED");
  }
...
function createWindowsCmdCommandV1(executable, args) {
  if (
    ...
    !executable.length ||
    WINDOWS_CMD_UNSAFE_TOKEN.test(executable)
  ) {
    fail("D2E4D_COMMAND_REJECTED");
  }

  const quotedArgs =
    args.map(quoteWindowsCmdTokenV1);
...
function canonicalizeWindowsPathEnvironmentV1(environment) {
  const normalizedEnvironment = {
    ...environment,
  };

  const pathKeys =
    Object.keys(normalizedEnvironment).filter(
  ...
  const preferredPathKey =
    pathKeys.find(
      (key) =>
        key === "PATH" &&
        typeof normalizedEnvironment[key] === "string" &&
        normalizedEnvironment[key].length > 0,
    ) ??
    pathKeys.find(
      (key) =>
        key === "Path" &&
  ...
  const pathValue =
    preferredPathKey === undefined
      ? undefined
      : normalizedEnvironment[preferredPathKey];

  for (const key of pathKeys) {
    delete normalizedEnvironment[key];
  }
    ...
    const result =
      await this.#execFile(
        ...
        resolvedArgs,
        {
          ...
          env:
            isWindowsCmdBridge
              ? canonicalizeWindowsPathEnvironmentV1(
                  options.environment === undefined
                    ? this.#environment
                    : previewCertificationEnvironmentV1(
                        this.#environment,
                        options.environment,
                      ),
                )
...
export class RealVercelPreviewCeremonyAdapterV1 {
  ...
  async deployOnce() {
    ...
    const certifiedBuildEnvironment =
      Object.freeze({
        VITE_AURA_RUNTIME_ENVIRONMENT: "PREVIEW",
        VITE_AI_UX_02D2E4_CONTROL_PROOF_DIGEST_V1:
          this.#controlProofDigest,
      });
    ...
    await this.#executor.execute(
      ...
      {
        cwd: this.#releaseRoot,
        environment: certifiedBuildEnvironment,
      },
    );
...
export class PlaywrightCoreBrowserRuntimeV1 {
  ...
  constructor({
    ...
    if (
      ...
      fail("D2E4D_PROTECTION_BYPASS_REJECTED");
    }

    this.#executablePath = executablePath;
    this.#allowLocalHarness =

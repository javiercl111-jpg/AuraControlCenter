...
export class FirestoreAdaptiveCanaryControlPlaneV1 {
  ...
  async verifyActive(expected) {
    if (
      typeof expected?.policyVersion !== "string" ||
      typeof expected?.authoritativeTenantLocator !== "string" ||
      !expected.authoritativeTenantLocator
    ) {
      fail(
        "D2E4O_CONTROL_PLANE_ACTIVE_POLICY_VERIFY_INPUT_REJECTED",
      );
    }
    ...
    if (
      ...
      fail(
        "D2E4O_CONTROL_PLANE_ACTIVE_POLICY_VERIFY_REJECTED",
      );
    }

    return Object.freeze({
      status: "ACTIVE_POLICY_VERIFIED",
      policyVersion:
        current.active.policyVersion,
      fingerprint:
        fingerprint({
          version:
            "AI_UX_02D2E4O_ACTIVE_POLICY_VERIFY_V1",
          policyVersion:
            current.active.policyVersion,

...
test("RealVercelPreviewCeremonyAdapterV1 builds certified Preview before one prebuilt deploy", async () => {
  const calls = [];
  ...
  assert.deepEqual(
    ...
    [
      ...
      "preview",
      "--local-config",
      "vercel.preview-certification.json",
    ],
  );

  assert.deepEqual(
    calls[0].options.environment,
    {
  ...
  assert.deepEqual(
    ...
    [
      "inspect",
      "https://unit-live-custody.vercel.app",
      "--json",
    ],
  );

  assert.equal(
    calls.flatMap(({ args }) => args).includes("--prod"),
    false,
  );
  ...
  assert.equal(
    calls
      .slice(1)
      .some(({ args }) =>
        args.some((value) => value.includes(digest))),
    false,
  );
});

test("NodeProcessCommandExecutorV1 allows only exact transient Preview certification environment", async () => {
  const {
    NodeProcessCommandExecutorV1,
  } = await import(
    "../ai-ux-02d2e4-final-preview-ceremony.mjs"
  ...
  const runtimeBefore =
    process.env.VITE_AURA_RUNTIME_ENVIRONMENT;
    ...
    new NodeProcessCommandExecutorV1({
      environment: {
        ...
        VITE_AI_UX_02D2E4_CONTROL_PROOF_DIGEST_V1:
          "must-be-stripped-from-constructor",
      },
      execFile: async (
        executable,
        args,
  ...
  assert.equal(
    calls[0].options.env.SAFE_BASE,
    "preserved",
  );
  assert.equal(
    calls[0].options.env.VITE_AURA_RUNTIME_ENVIRONMENT,
    "PREVIEW",
  );
  assert.equal(
    calls[0].options.env
  ...
  await assert.rejects(
    ...
    /D2E4D_PREVIEW_CERTIFICATION_ENV_REJECTED/u,
  );
...
test("PlaywrightCoreBrowserRuntimeV1 sends Vercel protection headers without leaking the bypass to Chromium child env", async () => {
  const { createServer } =
  ...
  const runtime =
    new PlaywrightCoreBrowserRuntimeV1({
  ...
  assert.throws(
    ...
    /D2E4D_PROTECTION_BYPASS_REJECTED/u,
  );
});

test("NodeProcessCommandExecutorV1 strips Vercel automation bypass from every child environment", async () => {
  const secret =
    "C".repeat(32);
...
test("ceremony source carries bypass only through protected HTTP seams and never command arguments", async () => {
  const { readFile } =
    await import(
      "node:fs/promises"
    );
  ...
  assert.match(
    source,
    /"x-vercel-protection-bypass"/u,
  );

  assert.match(
    source,
    /"x-vercel-set-bypass-cookie"/u,
  );

  assert.match(
    source,
    /env:\s*safeEnvironment\(process\.env\)/u,
  );
...
import { test as c100r3Test } from "node:test";
import * as c100r3Assert from "node:assert/strict";
import {
  NodeProcessCommandExecutorV1 as C100R3Executor,
} from "../ai-ux-02d2e4-final-preview-ceremony.mjs";

c100r3Test(
  "C100-R3 canonicalizes Path to PATH for Windows .cmd child environment",
  async () => {

...
function certificationSidecar(overrides = {}, projectId = D2E4G_PREVIEW_PROJECT_ID) {
...
function boundary({
  ...
  const adapter = new ExistingPreviewDeploymentReadBackAdapterV1({
    ...
    ...(previewTarget === undefined ? {} : { previewTarget }),
    executor: {
      async execute(executable, args, options) {
        ...
        return {
          stdout: JSON.stringify({
            ...
            name: D2E4G_PREVIEW_PROJECT_NAME,
...
test("read-back accepts the Vercel CLI 54.20.1 inspect schema without weakening certification", async () => {
  ...
  const receipt = await adapter.readBack({
    traceId: "trace-readback-vercel-cli-54-schema-0001",
  });

  assert.equal(receipt.status, "READY");
  assert.equal(receipt.projectId, D2E4G_PREVIEW_PROJECT_ID);
  assert.equal(receipt.deploymentId, D2E4G_PREVIEW_DEPLOYMENT_ID);
  assert.equal(receipt.deploymentRevision, D2E4G_PREVIEW_DEPLOYMENT_ID);
  assert.equal(receipt.readyState, "READY");
  assert.equal(receipt.deploymentInvocations, 0);
  assert.equal(receipt.productionChanged, false);
  assert.equal(receipt.stagingChanged, false);
  assert.equal(Object.isFrozen(receipt), true);
...
test("read-back still rejects a mismatched project name in the real Vercel CLI schema", async () => {
  const { adapter } = boundary({
    provider: {
      project: undefined,
      projectId: undefined,
      name: "different-preview-project",
      deploymentRevision: undefined,
    },
  });

  await assert.rejects(
    () => adapter.readBack({ traceId: "trace-readback-project-mismatch-0001" }),
    (error) => error.code === "D2E4G_EXISTING_PREVIEW_REJECTED",
  );
});
...
test("DeploymentReadBackHttpReaderV1 adds only the Vercel automation bypass header and preserves redirect fail-closed", async () => {
  const {
    DeploymentReadBackHttpReaderV1,
  } = await import(
  ...
  const url =
    "https://unit-protected-preview.vercel.app/.well-known/certification.json";
  ...
  try {
    ...
    const reader =
      new DeploymentReadBackHttpReaderV1({
        protectionBypass: secret,
      });

    await reader.readJson(url);
    ...
    const unprotectedReader =
      new DeploymentReadBackHttpReaderV1();

    await unprotectedReader.readJson(url);
    ...
    assert.throws(
      () =>
        new DeploymentReadBackHttpReaderV1({
          protectionBypass: "invalid",
        }),
      /D2E4G_PROTECTION_BYPASS_REJECTED/u,
    );
  }
  finally {
    globalThis.fetch =
...
test("read-back separates Vercel project name from immutable project id", async () => {
  const realProjectId = "prj_C110R12CDualIdentity0001";

  const sidecar =
    certificationSidecar({}, realProjectId);
  ...
  const receipt = await adapter.readBack({
    traceId: "trace-readback-dual-project-identity-0001",
  });

  assert.equal(receipt.status, "READY");
  assert.equal(receipt.projectId, realProjectId);
  assert.equal(receipt.deploymentId, D2E4G_PREVIEW_DEPLOYMENT_ID);
  assert.equal(receipt.productionChanged, false);
  assert.equal(receipt.stagingChanged, false);
});

...
import { readFile as c110r12cR5ReadFile } from "node:fs/promises";
import { test as c110r12cR5Test } from "node:test";
import c110r12cR5Assert from "node:assert/strict";

c110r12cR5Test("C110-R12C-R5 D2E4J forwards projectName into D2E4G validator", async () => {
  const source = await c110r12cR5ReadFile(
    new URL("../ai-ux-02d2e4j-preview-ceremony-composition.mjs", import.meta.url),
    "utf8",
  );

  const block = source.match(
    /assertD2E4GPreviewTargetV1\(\{([\s\S]*?)\}\);/u,
  );

  c110r12cR5Assert.ok(block);

  c110r12cR5Assert.match(
    block[1],
    /projectName:\s*requiredText\(\s*source,\s*"projectName",\s*\),/u,
  );
});
c110r12cR5Test("C110-R14 control target derives projectName from deployment projectName", async () => {
  const source = await c110r12cR5ReadFile(
    new URL("../ai-ux-02d2e4j-preview-ceremony-composition.mjs", import.meta.url),
    "utf8",
  );
  ...
  const block = source.match(
    /const controlTarget =\s*Object\.freeze\(\{([\s\S]*?)\}\);/u,
  );
  ...
  c110r12cR5Assert.match(
    block[1],
    /projectName:\s*deploymentTarget\.projectName,/u,
  );

  c110r12cR5Assert.doesNotMatch(
    block[1],
    /projectName:\s*deploymentTarget\.projectId,/u,
  );
});

...
export class LiveAdaptiveCanaryCeremonyAdapterV1 {
  ...
  async prepare(change) {
    ...
    if (
      change?.expectedCurrentPolicyVersion ===
        candidate.policyVersion
    ) {
      if (
        typeof this.#controlPlane?.verifyActive !==
          "function"
      ) {
        fail(
          "D2E4T_ACTIVE_POLICY_VERIFY_DEPENDENCY_REJECTED",
        );
      }
      ...
      if (
        verified?.status !== "ACTIVE_POLICY_VERIFIED" ||
        verified?.policyVersion !==
          candidate.policyVersion ||
        typeof verified?.fingerprint !== "string" ||
        verified?.logicalMutations !== 0 ||
        verified?.pointerPolicyAuditMatch !== true
      ) {
        fail(
          "D2E4T_ACTIVE_POLICY_VERIFY_REJECTED",
        );
      }

...
const emulatorConfig = resolve(__dirname, "firebase.capabilities.json");
const npxExecutable = process.platform === "win32" ? "npx.cmd" : "npx";

assert.equal(process.version, "v20.20.2", `Capability certification requires Node v20.20.2; received ${process.version}`);
...
const rawTestCommand =
  "npm exec -- vitest run --config functions/tests/emulator/capabilities/vitest.config.ts";

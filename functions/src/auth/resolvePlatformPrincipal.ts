...
export async function resolvePlatformPrincipal(
  ...
  if (uidDoc.exists) {
    ...
    if (!isActive) {
      throw new HttpsError("permission-denied", "Acceso deshabilitado para esta cuenta.");
    }
  ...
  if (!authContext.token?.email) {
    throw new HttpsError("permission-denied", "No se encontró registro de administrador para este usuario.");
  }
  ...
  if (emailDoc.exists) {
    ...
    if (emailQuery.size > 1) {
      throw new HttpsError("failed-precondition", "Conflicto de correo electrónico: múltiples cuentas coinciden.");
    }
  ...
  if (data.identityStatus === "INACTIVE_ALIAS") {
    throw new HttpsError("permission-denied", "Esta cuenta ha sido consolidada y no está activa como identidad administrativa.");
  }
  ...
  if (data.uid && data.uid !== callerUid) {
    throw new HttpsError("failed-precondition", "El correo ya está vinculado a otro identificador único.");
  }
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      if (newDoc.exists) {
        throw new HttpsError("failed-precondition", "Conflicto: ya existe un administrador registrado con este identificador único (UID).");
      }

      // Re-fetch the targetDoc inside transaction to ensure atomic execution
      const legacyDoc = await transaction.get(targetDoc!.ref);
      if (!legacyDoc.exists) {
        throw new HttpsError("not-found", "El registro heredado no existe al momento de la transacción.");
      }
      ...
      if (
        (legacyData.uid && legacyData.uid !== callerUid) ||
        (legacyData.authUid && legacyData.authUid !== callerUid)
      ) {
        throw new HttpsError("failed-precondition", "Conflicto: El registro heredado está vinculado a otro identificador único.");
      }
    ...
    throw new HttpsError("failed-precondition", `Error en la reconciliación de identidad: ${err.message}`);
  }

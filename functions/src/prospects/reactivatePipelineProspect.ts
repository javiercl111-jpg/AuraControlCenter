 ...
 */
export const reactivatePipelineProspect = onCall(
  {
  ...
  async (request) => {
    ...
    if (!companyId || !idempotencyKey) {
      throw new HttpsError("invalid-argument", "Los campos companyId e idempotencyKey son obligatorios.");
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (!companySnap.exists) {
        throw new HttpsError("not-found", "El prospecto no existe.");
      }

      const company = companySnap.data()!;
      if (company.status !== "DISCARDED") {
        throw new HttpsError("failed-precondition", "El prospecto no está en estado descartado.");
      }

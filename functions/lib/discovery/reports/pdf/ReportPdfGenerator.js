...
class ReportPdfGenerator {
    ...
    static async generateExternalRadiografia(data, branding) {
        ...
        if (data.maturityScore !== undefined) {
            ...
            PdfComponents_1.PdfComponents.drawMaturityGauge(doc, ds, layout, data.maturityScore, ds.pageWidth / 2, layout.yPos + 35, 30);
        ...
        if (data.keyFindings && data.keyFindings.length > 0) {
            ...
            data.keyFindings.forEach((finding, idx) => {
                PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, `Hallazgo ${idx + 1}`, finding, "finding", pageHeader);
            });
        }
        // ALLOW_FULL DETAILS
        if (data.deliveryLevel === "ALLOW_FULL") {
            if (data.operationalRisks && data.operationalRisks.length > 0) {
                ...
                data.operationalRisks.forEach((risk, idx) => {
                    PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, `Riesgo ${idx + 1}`, risk, "risk", pageHeader);
                });
            }
            if (data.opportunities && data.opportunities.length > 0) {
                ...
                data.opportunities.forEach((opp, idx) => {
                    PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, `Oportunidad ${idx + 1}`, opp, "opportunity", pageHeader);
                });
            }
    ...
    static async generateInternalBriefing(data, branding) {
        ...
        if (data.keyFindings && data.keyFindings.length > 0) {
            ...
            layout.yPos += 5;
            data.keyFindings.forEach((finding, idx) => {
                PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, `Señal ${idx + 1}`, finding, "finding", pageHeader);
            });
        }
        if (data.operationalRisks && data.operationalRisks.length > 0) {
            layout.ensureSpace(20, pageHeader);

...
const ReportDesignSystem_1 = require("./ReportDesignSystem");
const PdfLayoutEngine_1 = require("./PdfLayoutEngine");
const PdfComponents_1 = require("./PdfComponents");
class ReportPdfGenerator {
    static addHeader(doc, ds, folio) {
    ...
    static addFooter(doc, ds, pageNum, totalPages) {
    ...
    static addWatermark(doc, ds, company, contact) {
    ...
    static async generateExternalRadiografia(data, branding) {
        ...
        const ds = new ReportDesignSystem_1.ReportDesignSystem(branding);
        const layout = new PdfLayoutEngine_1.PdfLayoutEngine(doc, ds);
        const pageHeader = () => {
        ...
        if (data.maturityScore !== undefined) {
            ...
            layout.yPos += 15;
            PdfComponents_1.PdfComponents.drawMaturityGauge(doc, ds, layout, data.maturityScore, ds.pageWidth / 2, layout.yPos + 35, 35);
        ...
        if (data.keyFindings && data.keyFindings.length > 0) {
            ...
            data.keyFindings.forEach((finding) => {
                PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, "Observación de Campo", finding, "finding", pageHeader);
            });
        }
        // ALLOW_FULL DETAILS
        if (data.deliveryLevel === "ALLOW_FULL") {
            if (data.operationalRisks && data.operationalRisks.length > 0) {
                ...
                data.operationalRisks.forEach((risk) => {
                    PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, "Riesgo Detectado", risk, "risk", pageHeader);
                });
            }
            if (data.opportunities && data.opportunities.length > 0) {
                ...
                data.opportunities.forEach((opp) => {
                    PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, "Iniciativa Estratégica", opp, "opportunity", pageHeader);
                });
            }
            if (data.roadmap && data.roadmap.length > 0) {
                ...
                layout.yPos += 10;
                PdfComponents_1.PdfComponents.drawRoadmapTimeline(doc, ds, layout, data.roadmap, pageHeader);
            }
            // CIERRE
    ...
    static async generateInternalBriefing(data, branding) {
        ...
        if (data.keyFindings && data.keyFindings.length > 0) {
            ...
            layout.yPos += 10;
            data.keyFindings.forEach((finding) => {
                PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, "Señal Detectada", finding, "finding", pageHeader);
            });
        }
        if (data.operationalRisks && data.operationalRisks.length > 0) {
            layout.ensureSpace(20, pageHeader);
            ...
            layout.yPos += 10;
            data.operationalRisks.forEach((risk) => {
                PdfComponents_1.PdfComponents.drawCard(doc, ds, layout, "Alerta", risk, "risk", pageHeader);
            });
        }
        // ADD FOOTERS TO ALL PAGES (Except cover)
        const pageCount = doc.internal.getNumberOfPages();

...
const mexicoStates_1 = require("../../src/types/mexicoStates");
...
async function main() {
    ...
    async function runQueryREST(stateName) {
    ...
    async function writeMetadataREST(stateCode, fields) {
    ...
    try {
        ...
        const proposals = [];
        for (const state of mexicoStates_1.MEXICO_STATES) {
        ...
        if (proposals.length === 0) {
            ...
            proposals.forEach(p => {
                const stateObj = mexicoStates_1.MEXICO_STATES.find(s => s.code === p.code);
...
main();
//# sourceMappingURL=backfillStateMetadata.js.map
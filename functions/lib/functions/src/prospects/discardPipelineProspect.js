...
exports.discardPipelineProspect = void 0;
 ...
 */
exports.discardPipelineProspect = (0, https_1.onCall)({
    ...
    if (!assignmentId || !reasonCode || !idempotencyKey) {
        throw new https_1.HttpsError("invalid-argument", "Los campos assignmentId, reasonCode e idempotencyKey son obligatorios.");
    }
    ...
    if (!validReasonCodes.includes(reasonCode)) {
        throw new https_1.HttpsError("invalid-argument", "Código de descarte inválido.");
    }
    ...
    if (!allowedRoles.includes(caller.role)) {
        throw new https_1.HttpsError("permission-denied", "Rol insuficiente para descartar prospectos.");
    }
    if (caller.role === "SALES_ADVISOR" && caller.advisorId) {
        ...
        if (advDoc.exists) {
            ...
            if (advStatus === "INACTIVE" || advStatus === "SUSPENDED") {
                throw new https_1.HttpsError("permission-denied", "El asesor se encuentra inactivo o suspendido.");
            }
        }
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (!assignmentSnap.exists) {
            throw new https_1.HttpsError("not-found", "Asignación de pipeline comercial no encontrada.");
        }
        const assignment = assignmentSnap.data();
        if (assignment.status !== "ACTIVE" && assignment.status !== "CONTACTED") {
            throw new https_1.HttpsError("failed-precondition", "La asignación debe estar activa para poder descartarse.");
        }
        ...
        if (!isAdmin) {
            ...
            if (advisorDoc.data()?.uid !== callerUid) {
                throw new https_1.HttpsError("permission-denied", "No tienes permisos para descartar este prospecto asignado.");
            }
        ...
        if (!companySnap.exists) {
            throw new https_1.HttpsError("not-found", "El prospecto comercial no existe.");
        }
...
//# sourceMappingURL=discardPipelineProspect.js.map
...
exports.processProspectLifecycle = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
const LifecycleAdapter_1 = require("./lifecycle/LifecycleAdapter");
/**
 ...
 */
exports.processProspectLifecycle = functions.https.onCall(async (data, context) => {
    ...
    for (const doc of querySnap.docs) {
        ...
        // Evaluate pure logic
        const output = LifecycleAdapter_1.LifecycleAdapter.evaluate(prospect, currentDate, [], // For pure time decay, we don't need the full attempt array, count is on the prospect
        ...
        if (output.nextStatus !== prospect.lifecycleStatus || output.eventsToEmit.length > 0) {
            await db.runTransaction(async (t) => {
                ...
                if (output.noResponseSince)
                    updates.noResponseSince = admin.firestore.Timestamp.fromDate(output.noResponseSince);
                if (output.nurtureUntil)
                    updates.nurtureUntil = admin.firestore.Timestamp.fromDate(output.nurtureUntil);
                if (output.archivedAt)
                    updates.archivedAt = admin.firestore.Timestamp.fromDate(output.archivedAt);
                if (output.nextContactAt)
                    updates.nextContactAt = admin.firestore.Timestamp.fromDate(output.nextContactAt);
                ...
                for (const evt of output.eventsToEmit) {
                    ...
                    const eventToWrite = { ...evt };
                    eventToWrite.createdAt = admin.firestore.Timestamp.fromDate(evt.createdAt);
...
//# sourceMappingURL=processProspectLifecycle.js.map
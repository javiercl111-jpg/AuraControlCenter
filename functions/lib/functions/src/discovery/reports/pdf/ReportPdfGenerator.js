...
exports.ReportPdfGenerator = void 0;
const jspdf_1 = require("jspdf");
const fs = require("fs");
const path = require("path");
const aura_executive_documents_1 = require("aura-executive-documents");
class ReportPdfGenerator {
    ...
    static pageHeader(doc, folio) {
        aura_executive_documents_1.AuraPdfComponents.addTopBrandBar(doc);
        doc.setFont("helvetica", "bold");
        doc.setFontSize(8);
        doc.setTextColor(255, 255, 255);
        doc.text("AURA INTELLIGENCE", aura_executive_documents_1.AuraDesignTokens.margin.left, 10);
        if (folio) {
            doc.setFont("helvetica", "normal");
            doc.setTextColor(aura_executive_documents_1.AuraDesignTokens.colors.textMuted[0], aura_executive_documents_1.AuraDesignTokens.colors.textMuted[1], aura_executive_documents_1.AuraDesignTokens.colors.textMuted[2]);
            doc.text(`FOLIO: ${folio}`, aura_executive_documents_1.AuraDesignTokens.pageWidth - aura_executive_documents_1.AuraDesignTokens.margin.right, 10, { align: "right" });
        }
    }
    static addWatermark(doc, company, contact) {
        doc.saveGraphicsState();
        doc.setTextColor(230, 235, 240);
        doc.setFontSize(8);
        doc.setFont("helvetica", "bold");
        doc.text("CONFIDENCIAL — EXCLUSIVO PARA EL DESTINATARIO", aura_executive_documents_1.AuraDesignTokens.margin.left, aura_executive_documents_1.AuraDesignTokens.pageHeight - aura_executive_documents_1.AuraDesignTokens.margin.bottom + 10);
        doc.restoreGraphicsState();
    }
    static async generateExternalRadiografia(data, branding) {
        const doc = new jspdf_1.jsPDF("p", "mm", "a4");
        const ds = aura_executive_documents_1.AuraDesignTokens;
        const layout = new aura_executive_documents_1.AuraPdfLayout(doc);
        ...
        // PAGE 1: COVER
        aura_executive_documents_1.AuraPdfComponents.drawCoverPage(doc, assets, "RADIOGRAFÍA EMPRESARIAL", "AURA INTELLIGENCE", data.companyName, data.contactName);
        ...
        if (data.maturityScore !== undefined) {
            ...
            layout.yPos += 15;
            aura_executive_documents_1.AuraPdfComponents.drawMaturityGauge(doc, layout, data.maturityScore, ds.pageWidth / 2, layout.yPos + 35, 35);
        ...
        if (data.keyFindings && data.keyFindings.length > 0) {
            ...
            data.keyFindings.forEach((finding) => {
                aura_executive_documents_1.AuraPdfComponents.drawCard(doc, layout, "Observación de Campo", finding, "finding", pageHeader);
            });
        }
        // ALLOW_FULL DETAILS
        if (data.deliveryLevel === "ALLOW_FULL") {
            if (data.operationalRisks && data.operationalRisks.length > 0) {
                ...
                data.operationalRisks.forEach((risk) => {
                    aura_executive_documents_1.AuraPdfComponents.drawCard(doc, layout, "Riesgo Detectado", risk, "risk", pageHeader);
                });
            }
            if (data.opportunities && data.opportunities.length > 0) {
                ...
                data.opportunities.forEach((opp) => {
                    aura_executive_documents_1.AuraPdfComponents.drawCard(doc, layout, "Iniciativa Estratégica", opp, "opportunity", pageHeader);
                });
            }
            if (data.roadmap && data.roadmap.length > 0) {
                ...
                layout.yPos += 10;
                aura_executive_documents_1.AuraPdfComponents.drawRoadmapTimeline(doc, layout, data.roadmap, pageHeader);
            }
            // CIERRE
        ...
        for (let i = 2; i <= pageCount; i++) {
            doc.setPage(i);
            aura_executive_documents_1.AuraPdfComponents.addFooter(doc, i - 1, pageCount - 1);
        }
        (0, aura_executive_documents_1.validatePdfBounds)(doc);
        return Buffer.from(doc.output("arraybuffer"));
    }
    static async generateInternalBriefing(data, branding) {
        const doc = new jspdf_1.jsPDF("p", "mm", "a4");
        const ds = aura_executive_documents_1.AuraDesignTokens;
        const layout = new aura_executive_documents_1.AuraPdfLayout(doc);
        const assets = this.getAssets();
        ...
        // PAGE 1: COVER
        aura_executive_documents_1.AuraPdfComponents.drawCoverPage(doc, assets, "EXECUTIVE BRIEFING", "AURA INTELLIGENCE", data.companyName, data.contactName);
        ...
        if (data.keyFindings && data.keyFindings.length > 0) {
            ...
            layout.yPos += 10;
            data.keyFindings.forEach((finding) => {
                aura_executive_documents_1.AuraPdfComponents.drawCard(doc, layout, "Señal Detectada", finding, "finding", pageHeader);
            });
        }
        if (data.operationalRisks && data.operationalRisks.length > 0) {
            layout.ensureSpace(20, pageHeader);
            ...
            layout.yPos += 10;
            data.operationalRisks.forEach((risk) => {
                aura_executive_documents_1.AuraPdfComponents.drawCard(doc, layout, "Alerta", risk, "risk", pageHeader);
            });
        }
        // ADD FOOTERS TO ALL PAGES (Except cover)
        const pageCount = doc.internal.getNumberOfPages();
...
exports.ReportPdfGenerator = ReportPdfGenerator;
//# sourceMappingURL=ReportPdfGenerator.js.map
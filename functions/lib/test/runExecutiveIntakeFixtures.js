...
exports.runExecutiveIntakeFixtures = runExecutiveIntakeFixtures;
...
function runExecutiveIntakeFixtures() {
    ...
    function simulateValidation(payload) {
    ...
    try {
        ...
        const hash = (0, idempotencyHelper_1.generateRequestHash)(payload);
        assert(hash.length > 0, "5. Datos demográficos completos (Hash Generado)");
    }
    catch (e) {
        assert(false, "5. Datos demográficos completos");
    }
    ...
    try {
        ...
        assert(e.message === "INVALID_INPUT", "6. privacyConsent false (Lanzó INVALID_INPUT)");
    }
    ...
    try {
        ...
        assert(e.message === "INVALID_INPUT", "7. diagnosticDeliveryConsent false (Lanzó INVALID_INPUT)");
    }
    ...
    try {
        ...
        assert(false, "8. marketingConsent false permitido");
    }
    ...
    try {
        ...
        assert(false, "9. followUpConsent false permitido");
    }
    ...
    try {
        ...
        assert(false, "10. Payload legacy consent");
    }
    // 11 & 12 Idempotency checks logic
    const key = "submit-1";
    const p1 = buildPayload({ idempotencyKey: key });
    const h1 = (0, idempotencyHelper_1.generateRequestHash)(p1);
    const p2 = buildPayload({ idempotencyKey: key });
    const h2 = (0, idempotencyHelper_1.generateRequestHash)(p2);
    ...
    try {
        simulateValidation({});
        assert(false, "14. Payload malformado (Debería fallar)");
    }
    catch (e) {
        assert(e.message === "INVALID_INPUT", "14. Payload malformado");
    }
...
//# sourceMappingURL=runExecutiveIntakeFixtures.js.map